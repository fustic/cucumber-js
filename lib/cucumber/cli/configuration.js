function Configuration(argv) {
  ...
  var self = {
    ...
    getAstFilter: function getAstFilter() {
      ...
      rules = rules.concat(self.getModularPartitioningRules());
    ...
    getModularPartitioningRules: function getModularPartitioningRules() {
      var rules = [];
      var result = argumentParser.getModularPartitioningOptions();
      if (result) {
          rules.push(Cucumber.Ast.Filter.ModularPartitioning(result.parts, result.remainder));
        }
      return rules;
    },
